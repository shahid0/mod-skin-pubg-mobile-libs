...
ProcMap KittyMemory::getLibraryMap(const char *libraryName) {     
    ...
    FILE *fp = fopen(lj, OBFUSCATE("rt"));     
    if (fp != NULL) {         
        int count = 0; // đếm số lần tìm thấy thư viện         
        while (fgets(line, sizeof(line), fp)) {             
            if (strstr(line, libraryName)) {
            if (_Arm64) {
            if (libraryName == "libtdmodgame.so") {
                count++;                 
                if (count == 2) { // nếu tìm thấy lần thứ 2, lấy thông tin ProcMap                     
